...
export default function Home() {
  ...
  useEffect(() => {
    fetch('/data/students.json')
      ...
      .then((d: Student[]) => {
        ...
        const named = d.map((s, i) => ({
          ...s,
          persona: ['Analytical', 'Balanced', 'Hands-on'][labels[i] % 3],
        }));
        setData(named);
      });
  }, []);
  ...
  useEffect(() => {
    ...
    const corrs = (['comprehension', 'attention', 'focus', 'retention'] as const)
      .map(k => {
        const xs = data.map(s => s[k]);
  ...
  return (
    <div className="container">
      <Head>
        <title>Cognitive Skills & Student Performance Dashboard</title>
      </Head>

      {/* ==== Updated header with badge + subtitle ==== */}
      <h1 style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
        🧠 Cognitive Skills & Student Performance
        <span className="badge">Dashboard</span>
      </h1>
      <p className="muted" style={{ marginTop: 2 }}>
        Synthetic dataset • Correlations • ML prediction • Personas
      </p>
